...
class DropoutTrainer(Framework):
    def __init__(self, dataset_train,
        ...
        self.batch_size = 32  # default 128
        self.learning_rate = 1e-3
        self.num_epochs = 100
        self.weight_decay = 0.01
        our_seed = torch.manual_seed(0) # set seed for reproducibility

        # c_1. Initialize the MC_Dropout network and optimizer here
        # You can check the Dummy Trainer above for intuition about what to do
        self.network = MNISTNet(in_features=784,
                                out_features=10,
                                dropout_p=0.4,
    ...
    def predict_probabilities(self, x: torch.Tensor, num_sample=100) -> torch.Tensor:
        ...
        prediction_list = [F.softmax(self.network(x), dim=1) for _ in range(100)]
        estimated_probability = torch.stack(prediction_list).mean(dim=0)
        print("estimated_probability:", estimated_probability)
        assert estimated_probability.shape == (x.shape[0], 10)
        return estimated_probability
...
class EnsembleTrainer(Framework):
    def __init__(self, dataset_train, *args, **kwargs):
        ...
        self.linear_relu_stack1 = nn.Sequential(nn.Linear(in_features=784, out_features=512),
                                                nn.ReLU(),
                                                nn.Linear(in_features=512, out_features=512),
                                                nn.ReLU(),
                                                nn.Linear(in_features=512, out_features=10))

        self.linear_relu_stack2 = nn.Sequential(nn.Linear(in_features=784, out_features=784),
                                                nn.ReLU(),
                                                nn.Linear(in_features=512, out_features=512),
                                                nn.ReLU(),
                                                nn.Linear(in_features=512, out_features=10))

        self.linear_relu_stack3 = nn.Sequential(nn.Linear(in_features=784, out_features=784),
                                                nn.ReLU(),
                                                nn.Linear(in_features=512, out_features=512),
                                                nn.ReLU(),
                                                nn.Linear(in_features=512, out_features=10))

        self.EnsembleNetworks = [self.linear_relu_stack1,
                                 self.linear_relu_stack2,
                                 self.linear_relu_stack3]
        self.train_loader = torch.utils.data.DataLoader(dataset_train,
                                                        batch_size=self.batch_size,
                                                        shuffle=True,
                                                        drop_last=True)
        self.optimizer1 = torch.optim.Adam(self.EnsembleNetworks[0].parameters(), lr=self.learning_rate)
        self.optimizer2 = torch.optim.Adam(self.EnsembleNetworks[1].parameters(), lr=self.learning_rate)
        self.optimizer3 = torch.optim.Adam(self.EnsembleNetworks[2].parameters(), lr=self.learning_rate)

    # noinspection DuplicatedCode
    def train(self):

        for network in self.EnsembleNetworks:  # iterating through each neural net
            ...
            for _ in progress_bar:
                for batch_idx, (batch_x, batch_y) in enumerate(self.train_loader):
                    ...
                    # .zero_grad() needed because per default pytorch does sum up the gradients from each update step
                    network.zero_grad()
